...
def i1(popi=popi):
    if request.method == 'GET':
        try:
            sw = Tovar.query.all()
            if sw == None or sw == []:
                baba = Cart(user_id=current_user.id, thing_id=1, quantity=1)
                ...
                admin.opis = sw[0].opis + ", товар: модная толстовка белая цена: 3000 руб"
                admin2.photo = sw[0].photo + ", https://i.ibb.co/J7qXqPh/photo-2024-04-13-19-46-34-2.jpg"
                db.session.commit()
                with open("add_to_kors.html", 'r', encoding='utf-8') as html_stream:
                    return html_stream.read()
            ...
            sw = Tovar.query.all()
            print(sw)
            return render_template("vxof_done.html")
    else:
        print(8)
    with open("add_to_kors.html", 'r', encoding='utf-8') as html_stream:
        return html_stream.read()
    ...
    if request.method == 'GET':
        try:
            ...
            if sw == None or sw == []:
                baba = Cart(user_id=current_user.id, thing_id=1, quantity=1)
                print(baba)
                bob = Tovar(id=current_user.id, opis="товар: модная толстовка черная цена: 3000 руб",
                            photo="https://i.ibb.co/vZgyWrj/photo-2024-04-13-19-46-34.jpg")
                print(bob)
                db.session.add(bob)
                db.session.add(baba)
                db.session.commit()
    ...
    if request.method == 'GET':
        try:
            ...
            if sw == None or sw == []:
                baba = Cart(user_id=current_user.id, thing_id=1, quantity=1)
                print(baba)
                bob = Tovar(id=current_user.id, opis="товар: модные стринги белые цена: 3000 руб",
                            photo="https://i.ibb.co/s5LGJnD/photo-2024-04-13-19-32-50.jpg")
                print(bob)
                db.session.add(bob)
                db.session.add(baba)
                db.session.commit()
    ...
    if request.method == 'GET':
        try:
            ...
            if sw == None or sw == []:
                baba = Cart(user_id=current_user.id, thing_id=1, quantity=1)
                print(baba)
                bob = Tovar(id=current_user.id, opis="товар: модные стринги черные цена: 3000 руб",
                            photo="https://i.ibb.co/16Kx2JC/photo-2024-04-13-19-32-50-2.jpg")
                print(bob)
                db.session.add(bob)
                db.session.add(baba)
                db.session.commit()
    ...
    if request.method == 'GET':
        try:
            ...
            if sw == None or sw == []:
                baba = Cart(user_id=current_user.id, thing_id=1, quantity=1)
                print(baba)
                bob = Tovar(id=current_user.id, opis="товар: модная футболка белая цена: 3000 руб",
                            photo="https://i.ibb.co/Px80n8M/photo-2024-04-13-19-32-50-4.jpg")
                print(bob)
                db.session.add(bob)
                db.session.add(baba)
                db.session.commit()
    ...
    if request.method == 'GET':
        try:
            ...
            if sw == None or sw == []:
                baba = Cart(user_id=current_user.id, thing_id=1, quantity=1)
                print(baba)
                bob = Tovar(id=current_user.id, opis="товар: модная кружка уауауа цена: 3000 руб",
                            photo="https://i.ibb.co/5FfMCM2/photo-2024-04-14-21-33-29.jpg")
                print(bob)
                db.session.add(bob)
                db.session.add(baba)
                db.session.commit()
    ...
    if request.method == 'GET':
        try:
            ...
            if sw == None or sw == []:
                baba = Cart(user_id=current_user.id, thing_id=1, quantity=1)
                print(baba)
                bob = Tovar(id=current_user.id, opis="товар: страшная маска уауауа цена: 3000 руб",
                            photo="https://i.ibb.co/JBJptGH/photo-2024-04-13-13-06-01.jpg")
                print(bob)
                db.session.add(bob)
                db.session.add(baba)
                db.session.commit()
    ...
    if request.method == 'GET':
        try:
            ...
            if sw == None or sw == []:
                baba = Cart(user_id=current_user.id, thing_id=1, quantity=1)
                print(baba)
                bob = Tovar(id=current_user.id, opis="товар: чтооооо это чтооо цена: 3000 руб",
                            photo="https://i.ibb.co/cYhLN8m/199-20240424235154.png")
                print(bob)
                db.session.add(bob)
                db.session.add(baba)
                db.session.commit()
...
def i2():
    if request.method == 'POST':
        ...
        try:
            ...
            with open("reg_fail.html", 'r', encoding='utf-8') as html_stream:
                return html_stream.read()
    else:
        print(8)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if password == s[0].password:
                ...
                with open("vxode_done.html", 'r', encoding='utf-8') as html_stream:
                    return html_stream.read()
            else:
                with open("reg_fail.html", 'r', encoding='utf-8') as html_stream:
                    return html_stream.read()
...
def i14(res_busy=res_busy):
    ...
    if sw != []:
        ...
        res_busy = []
        return render_template("korza.html")
    # for j in res_busy:
    # print(j)
...
@app.route("/fin_buy")
def i31(res_busy=res_busy):
    sw = Tovar.query.all()
    print(sw)
    res_busy.clear()
    try:
        db.session.query(Tovar).filter(Tovar.opis == sw[0].opis, Tovar.photo == sw[0].photo).delete()
        #admin = Tovar.query.filter_by(opis=sw[0].opis).first()
        #admin2 = Tovar.query.filter_by(photo=sw[0].photo).first()
        #admin.opis = ""
        #admin2.photo = ""
        ...
        db.session.rollback()
        return('У вас нету товаров чтоб их оплатить!')
    res_busy.clear()
